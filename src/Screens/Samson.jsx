...
export const Samson = () => {
	const courtRatings = JSON.parse(localStorage.getItem("samson_Condition"));
	const savedCourts = JSON.parse(localStorage.getItem("saved_Courts"));

	const [saved, setSaved] = useState(savedCourts.samson);

	const save_button = () => {
		if (!saved) {
			savedCourts.samson = true;
	...
	return (
		<div className="bg-white flex flex-row justify-center w-full">
			<div className="bg-white w-[394px] h-[852px] relative">
				...
				{!saved && (
					<img
						className="absolute transition-all duration-300 hover:w-[25px] transition-all duration-300 hover:h-[31px] transition-all duration-300 hover:top-[101px] w-[23px] h-[29px] top-[102px] left-[240px]"
						alt="Bookmark empty"
						onClick={save_button}
